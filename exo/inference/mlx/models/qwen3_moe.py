...
from mlx_lm.models.qwen3_moe import Qwen3MoeDecoderLayer, ModelArgs
...
class ModelArgs(ModelArgs):
  ...
  def __post_init__(self):
    # Note: Don't call super().__post_init__() as Qwen3 MOE doesn't have it
  ...
  def __init__(self, args: ModelArgs):
    ...
    for i in range(self.num_hidden_layers):
      if self.args.shard.start_layer <= i <= self.args.shard.end_layer:
        self.layers.append(Qwen3MoeDecoderLayer(args=args, layer_idx=i))
      else:
...
class Model(nn.Module):
  ...
  def sanitize(self, weights):
    ...
    has_individual_experts = any(
      "mlp.experts." in key and ".up_proj.weight" in key 
      ...
      for l in range(self.args.num_hidden_layers):
        ...
        for n in ["up_proj", "down_proj", "gate_proj"]:
          ...
          for e in range(self.args.num_experts):
            expert_key = f"{prefix}.mlp.experts.{e}.{n}.weight"
          ...
          if expert_weights:
            shard_state_dict[f"{prefix}.mlp.switch_mlp.{n}.weight"] = mx.stack(expert_weights)
